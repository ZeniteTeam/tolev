...
import { Stagger, TabSwitch } from "../../../components";
...
export default function MetodoOnboardingScreen() {
  ...
  // A ordem de ataque é o método aplicado às dívidas reais do usuário — quem
  // ainda não cadastrou nenhuma vê o método explicado, sem a prévia.
  const ordem = metodo.ordenar(dividas);
  const total = dividas.reduce((s, d) => s + d.saldo, 0);
  const primeiroAlvo = ordem[0];
  ...
  return (
    <View className="flex-1 bg-bg">
      ...
      >
        {/* Cada etapa é uma tela dentro da tela: a `key` remonta o bloco,
            então avançar tem a mesma entrada de quem chega de fora. */}
        <TabSwitch tabKey={String(step)}>
          {step === 0 && (
            <>
              <View
              ...
              {ordem.length === 0 && (
                <View className="bg-surface rounded-[13px] px-3.5 py-[14px]" style={shadows.card}>
                  <Text className="text-[13px] text-muted leading-[19px] font-regular">
                    Cadastre suas dívidas para ver em que ordem este método atacaria cada uma.
                  </Text>
                </View>
              )}
                ...
                <>
                  ...
                  <View className="flex-row items-center gap-3 bg-surface rounded-[14px] px-4 py-4 mb-5" style={shadows.card}>
                    <View
                      ...
                      style={{ backgroundColor: primeiroAlvo.bankColor }}
                    >
                      {(() => {
                        const FirstIcon = primeiroAlvo.icon;
                        return <FirstIcon size={19} color="#fff" strokeWidth={2} />;
                      })()}
                    </View>
                    <View className="flex-1">
                      <Text className="text-[14px] font-semibold text-ink">{primeiroAlvo.nome}</Text>
                      <Text className="text-[12px] text-muted font-regular">
                        {brl(primeiroAlvo.saldo)} · {primeiroAlvo.juros.toFixed(1).replace(".", ",")}% a.m.
                      </Text>
                    </View>
                  </View>
                </>
