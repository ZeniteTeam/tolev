...
import { AlertCircle, CalendarClock, Check, TrendingUp } from "lucide-react-native";
...
import { brl, DIVIDAS_SEED } from "../constants/dividas";
...
export default function DividaDetalheScreen() {
    ...
    registrar.mutate(
      { idDivida: d.id, parcelas, valorPorParcela },
      {
        onSuccess: () => {
          ...
          Alert.alert("Pagamento registrado", `${parcelas.length} ${label} · ${brl(total)}.`);
        },
        onError: (err) =>
  ...
  const pago = Math.round(d.saldo * 0.35);
  const original = d.saldo + pago;
  const pctPago = Math.round((pago / original) * 100);
  const jurosMes = Math.round(d.saldo * (d.juros / 100));

  const historico = [
    { mes: "Set/2025", valor: d.min, status: "ok" as const },
    { mes: "Ago/2025", valor: d.min, status: "ok" as const },
    { mes: "Jul/2025", valor: Math.round(d.min * 0.6), status: "parcial" as const },
    ...
    <Screen bottomPad={40}>
      <View className="pt-1 mb-[18px]">
        ...
        >
          ...
          <Text className="text-[12px] font-semibold text-primary-700">{d.banco}</Text>
        </View>
        <Text className="font-bold text-[24px] leading-[28px] text-ink">{d.nome}</Text>
      </View>
      ...
      >
        ...
        <Text className="text-white text-[32px] leading-9 font-bold mt-1.5">{brl(d.saldo)}</Text>

        <View className="flex-row justify-between mt-3.5 mb-1.5">
          <Text className="text-white/90 text-[12px] font-regular">{brl(pago)} pagos</Text>
          <Text className="text-white/90 text-[12px] font-regular">{pctPago}% quitado</Text>
        </View>
        ...
        <View className="flex-row mt-4 pt-4 border-t border-t-white/[0.18] gap-3.5">
          <View className="flex-1">
            <Text className="text-white/[0.78] text-[11px] font-regular">Juros a.m.</Text>
          ...
          <View className="flex-1">
            ...
            <Text className="text-white text-[17px] font-bold mt-0.5">{brl(d.min)}</Text>
          </View>
        </View>
      </LinearGradient>
      ...
      <View className="flex-row gap-3 mb-4">
        <View className="flex-1 bg-surface rounded-[18px] p-[18px]" style={shadows.card}>
          ...
          <Text className="font-bold text-[18px] text-ink mt-3">Set/2027</Text>
          <Text className="text-[11px] text-muted mt-0.5 font-regular">quitação no ritmo atual</Text>
        </View>
        <View className="flex-1 bg-surface rounded-[18px] p-[18px]" style={shadows.card}>
          ...
          <Text className="font-bold text-[18px] text-coral-500 mt-3">{brl(jurosMes)}</Text>
          <Text className="text-[11px] text-muted mt-0.5 font-regular">juros neste mês</Text>
        </View>
      </View>

      <View className="bg-surface rounded-[18px] p-5 mb-4" style={shadows.card}>
        <Text className="font-bold text-[16px] text-ink mb-3.5">Histórico de pagamentos</Text>
        {historico.map((h, i) => (
          <View
            key={h.mes}
            className="flex-row items-center gap-3 py-[11px]"
            style={i !== historico.length - 1 ? { borderBottomWidth: 1, borderBottomColor: "#F1F5F3" } : undefined}
          >
            <View
              className="w-8 h-8 rounded-sm items-center justify-center"
              style={{ backgroundColor: h.status === "ok" ? colors.primary[100] : "#FEE7E0" }}
            >
              {h.status === "ok" ? (
            ...
            <Text className="text-[14px] font-bold text-ink">{brl(h.valor)}</Text>
          </View>
        ))}
      </View>
